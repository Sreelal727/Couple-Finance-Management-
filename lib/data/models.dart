...
class Member implements SyncedRecord {
  ...
  static Member fromMap(Map<String, Object?> m) =>
      Member(meta: SyncMeta.fromMap(m), name: m['name'] as String, colorHex: m['color'] as String);

  Member copyWith({String? name, String? colorHex, SyncMeta? meta}) =>
      Member(meta: meta ?? this.meta, name: name ?? this.name, colorHex: colorHex ?? this.colorHex);
}
...
class Txn implements SyncedRecord {
  ...
  int get partnerOwesPaise => (type == TxnType.expense && isShared) ? amountPaise - payerSharePaise : 0;
...
class Budget implements SyncedRecord {
  ...
  @override
  Map<String, Object?> toMap() => {...meta.toMap(), 'category_id': categoryId, 'amount': amountPaise};
...
class Peer {
  ...
  Peer copyWith({String? name, String? lastAddress, int? lastSeenAt, int? lastSyncAt}) => Peer(
    deviceId: deviceId,
    memberId: memberId,
    name: name ?? this.name,
